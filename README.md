...
Probability & Statistics I & II, Biostatistics, Data Analysis (topics including descriptive & inferential statistics, probability distributions, variable types (qualitative, quantitative), distribution properties, discrete & continuous types, R functions, ANOVA, T-test, correlation, chi-Square, regression, etc.). Regression (linear, multiple, logistic), Probability distributions (Discrete, Binomial, Poisson, Continuous Uniform, Chi-squared, Student-t, Fisher,)  Exponential, Normal, Chi-Square), hypothesis testing using critical value (Probability, F Statistics, Coefficient) Analysis of Variance (ANOVA), data wrangling, Post Hoc Test. Data wrangling and handling. Data Visualization (BoxPlots, Scatter Plots).
...
selected function(mlv, library, lm, abline, pie, readRDS, glimpse(df), rbind, grep, heatmap, is.data.frame, is.numeric, is.matrix, is.na, ggscatter, cor.test, mean, mode, Median, table, read.csv, chisq.test, dbinom, pbinom, ppois, runif, qchisq, qt, qf, pexp, runif, aov, summary, TukeyHSD, rnorm, seq, plot, lines, t.test, dbinom, levels, ggplot). Normal Distribution Plot: Normal-Dis.png
Probability Distribution Functions.png
Scatter-Plot.pdf
Ttest.pdf
